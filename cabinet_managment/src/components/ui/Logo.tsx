...
export function Logo({ className, iconClassName, textClassName, showText = true }: LogoProps) {
  return (
    <Link to={ROUTES.LANDING} className={cn("flex items-center gap-2.5 transition-transform hover:scale-105 active:scale-95", className)}>
      <div className={cn("h-9 w-9 shrink-0", iconClassName)}>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" fill="none" className="h-full w-full">
          <defs>
            <linearGradient id="logoHeart" x1="12" y1="10" x2="54" y2="54" gradientUnits="userSpaceOnUse">
      ...
      {showText && (
        <span className={cn("text-[17px] font-bold tracking-tight text-gray-900 dark:text-slate-100", textClassName)}>
          DocNow
        </span>
      )}
    </Link>
